...
$proxies=@('dxgi.dll','winmm.dll','version.dll','winhttp.dll','wininet.dll','dbghelp.dll') | ForEach-Object {
    ...
    if(Test-Path -LiteralPath $candidate -PathType Leaf) {
        ...
        if($item.VersionInfo.ProductName -match 'OptiScaler|DLSS 5 Neural' -or $item.VersionInfo.FileDescription -match 'OptiScaler|DLSS 5 Neural') {$item.Name}
    }
}
...
Write-Host 'Mod instalado com sucesso! Abra o jogo e ative o NVIDIA DLSS nas opções de vídeo.'
